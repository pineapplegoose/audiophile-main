...
export const createProduct = mutation(
  async ({ db }: { db: DatabaseWriter }) => {
    const products = [
      {
        ...
        category: "speakers",
        moreLikeThis: ["ZX9 Speaker", "ZX7 Speaker"],
      ...
      },
      {
        ...
        price: 599,
        imageUrl: "/assets/product-yx1-earphones/desktop/image-product.jpg",
        gallery: [
    ...
    return { message: `${products.length} products seeded!` }
  }
)

export const updateProduct = mutation({
  args: {
    _id: v.id("products"), // required to know which product to update
    ...
    inTheBox: v.optional(
      v.array(v.object({ quantity: v.number(), item: v.string() }))
    ),
  ...
  handler: async (ctx, args) => {
    ...
    // Patch the product with only the provided fields
    await ctx.db.patch(_id, updates)

    return { message: "Product updated successfully", id: _id }
  },
})
